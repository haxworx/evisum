...
static void _evisum_ui_disk_graph_redraw(Win_Data *wd);
...
static Eina_Bool
_evisum_ui_disk_graph_objects_valid(Win_Data *wd)
{
...
static void
_evisum_ui_disk_legend_toggle_cb(void *data, Evas_Object *obj EINA_UNUSED,
                  void *event_info EINA_UNUSED)
{
   ...
   if (!entry || !entry->wd || !entry->legend_btn || !entry->legend_row)
     return;
   if (!_evisum_ui_disk_graph_objects_valid(entry->wd))
     return;
   entry->enabled = !entry->enabled;
   _evisum_ui_disk_legend_toggle_state_apply(entry);
   _evisum_ui_disk_graph_redraw(entry->wd);
}

static void
_evisum_ui_disk_history_legend_repack(Win_Data *wd)
{
   Eina_List *l;
   Disk_History *entry;
   int row = 0;
...
static void
_evisum_ui_disk_history_legend_add(Win_Data *wd, Disk_History *entry)
{
   ...
   entry->legend_pb = pb;
   _evisum_ui_disk_legend_toggle_state_apply(entry);
   _evisum_ui_disk_history_legend_repack(wd);
}
...
static void
_evisum_ui_disk_history_legend_update(Disk_History *entry, double used_percent,
...
static Disk_History *
_evisum_ui_disk_history_find_or_create(Win_Data *wd, const File_System *fs)
{
   Eina_List *l;
   Disk_History *entry;
   char keybuf[4096];
...
static void
_evisum_ui_disk_history_compact(Win_Data *wd)
{
   Eina_List *l, *l2;
   Disk_History *entry;
...
static void
_evisum_ui_disk_graph_redraw(Win_Data *wd)
{
   Eina_List *l;
   Disk_History *entry;
   int total, nseries;
   ...
   if (!_evisum_ui_disk_graph_objects_valid(wd))
     return;
                    ...
                    void *event_info EINA_UNUSED)
{
   ...
   _evisum_ui_disk_graph_redraw(wd);
}
   ...
   EINA_LIST_FOREACH(mounted, l, fs)
     {
        ...
        entry = _evisum_ui_disk_history_find_or_create(wd, fs);
        if (!entry) continue;
...
static void
_evisum_ui_disk_main_menu_dismissed_cb(void *data, Evas_Object *obj EINA_UNUSED,
                        void *event_info EINA_UNUSED)
{
   Win_Data *wd = data;
...
static void
_evisum_ui_disk_btn_menu_clicked_cb(void *data, Evas_Object *obj,
                     void *event_info EINA_UNUSED)
{
   Win_Data *wd = data;
   ...
   if (!wd->main_menu)
     {
        ...
        evas_object_smart_callback_add(wd->main_menu, "dismissed",
                                       _evisum_ui_disk_main_menu_dismissed_cb, wd);
     }
   else
     {
        evas_object_del(wd->main_menu);
...
evisum_ui_disk_win_add(Evisum_Ui *ui)
{
   ...
   evas_object_event_callback_add(wd->graph_bg, EVAS_CALLBACK_RESIZE, _evisum_ui_disk_graph_bg_resize_cb, wd);
   evas_object_event_callback_add(wd->graph_bg, EVAS_CALLBACK_MOVE, _evisum_ui_disk_graph_bg_resize_cb, wd);
   ...
   evas_object_smart_callback_add(btn, "clicked", _evisum_ui_disk_btn_menu_clicked_cb, wd);
   ...
   evas_object_event_callback_add(win, EVAS_CALLBACK_DEL, _evisum_ui_disk_win_del_cb, wd);
   evas_object_event_callback_add(win, EVAS_CALLBACK_MOVE, _evisum_ui_disk_win_move_cb, wd);
   evas_object_event_callback_add(win, EVAS_CALLBACK_RESIZE, _evisum_ui_disk_win_resize_cb, wd);
   evas_object_event_callback_add(win, EVAS_CALLBACK_MOUSE_MOVE, _evisum_ui_disk_win_mouse_move_cb, wd);
   elm_object_focus_allow_set(tb, EINA_TRUE);
   elm_object_focus_set(tb, EINA_TRUE);
   evas_object_event_callback_add(tb, EVAS_CALLBACK_KEY_DOWN, _evisum_ui_disk_win_key_down_cb, wd);
   evas_object_show(win);

   wd->thread = ecore_thread_feedback_run(_evisum_ui_disk_disks_poll,
                                          _evisum_ui_disk_disks_poll_feedback_cb,
                                          NULL,
                                          NULL,
                                          wd, 1);
}
